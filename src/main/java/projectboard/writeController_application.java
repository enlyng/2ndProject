...
@WebServlet("/plantcare/applicationwrite.do")
public class writeController_application extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (mr == null) {
			JSFunction.alertLocation(resp, "첨부 파일 제한 용량을 초과합니다.", "./applicationwrite.do");
			return;
		}
		...
		dto.setSubaddress(mr.getParameter("address_detail"));
		...
		int result = dao.ApplicationWrite(dto);
		//커넥션 풀에 자원 반납
		dao.close();
		...
		if (result == 1) {
			...
			JSFunction.alertLocation(resp, "전송 완료♥", "./Design.jsp");
//					resp.sendRedirect("./Design.jsp");
		} else {
			System.out.println("작성실패");
			//insert에 실패하면 작성페이지로 다시 이동한다.
			resp.sendRedirect("./applicationwrite.do");
		}
